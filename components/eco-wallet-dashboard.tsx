...
export function EcoWalletDashboard({ className, userId = "68a9267e8ab481725758febd" }: EcoWalletDashboardProps) {
  ...
  const loadEcoTokenBalance = async () => {
    const addressToUse = FIXED_WALLET_ADDRESS;
    setIsLoadingBalance(true);
    
    try {
      // Try local API first
      const response = await fetch(`/api/user/balance/${addressToUse}`);
      
      if (response.ok) {
        const data = await response.json();
        setEcoTokenBalance({
          balance: data.balance || '0',
          balanceFormatted: data.balanceFormatted || `${data.balance || '0'} EcoTokens`,
          lastUpdated: new Date().toLocaleString()
        });
      } else {
        // Fallback to direct backend service
        const fallbackResponse = await fetch(`http://localhost:3001/balance/${addressToUse}`);
        
        if (fallbackResponse.ok) {
          const fallbackData = await fallbackResponse.json();
          setEcoTokenBalance({
            balance: fallbackData.balanceRaw || fallbackData.balance || '0',
            balanceFormatted: fallbackData.balanceFormatted || `${fallbackData.balance || '0'} EcoTokens`,
            lastUpdated: new Date().toLocaleString()
          });
        } else {
          setEcoTokenBalance({
            balance: '0',
            balanceFormatted: '0 EcoTokens',
            lastUpdated: new Date().toLocaleString()
          });
        }
      }
